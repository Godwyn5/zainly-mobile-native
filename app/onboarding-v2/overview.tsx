...
// ─── palette — identical tokens to Splash/Welcome/Name/Greeting (kept
// local, not exported from those files, to avoid touching them) ────────────
...
export default function OnboardingOverviewScreen() {
  ...
  // as the greeting screen (wash, glow, drift, two faint motifs), so the
  // space itself never feels like it changed when this scene arrives. ─────
  ...
  // ── the one story this screen tells: a light progressively illuminating
  // a stylized, closed Qur'an — nothing else moves for its own sake. ──────
  const bookOpacity  = useRef(new Animated.Value(0)).current;
  const bookScale    = useRef(new Animated.Value(0.92)).current;
  const lightOpacity = useRef(new Animated.Value(0)).current;
  const lightScale   = useRef(new Animated.Value(0.5)).current;

  // ── title & subtitle — a single calm reveal each, never per-character ──
  const titleOpacity    = useRef(new Animated.Value(0)).current;
  const titleY           = useRef(new Animated.Value(14)).current;
  // subtitle uses a "luminous reveal" (opacity + gentle bloom scale) —
  // deliberately different from the title's rise, and from the mask/focus
  // families used on adjacent screens (see animation audit). ─────────────
  ...
  useEffect(() => {
    ...
    if (reduceMotion) {
      bookOpacity.setValue(1); bookScale.setValue(1);
      lightOpacity.setValue(0.5); lightScale.setValue(1);
      titleOpacity.setValue(1); titleY.setValue(0);
      subtitleOpacity.setValue(0.68); subtitleScale.setValue(1);
      schedule(() => finishScene(), 900);
      return;
    }

    schedule(() => runIllustration(), 260);
  ...
  function runIllustration() {
    Animated.parallel([
      ...
      Animated.timing(lightOpacity, {
        toValue: 0.55, duration: 900, delay: 180,
        easing: Easing.out(Easing.cubic), useNativeDriver: true,
      }),
      Animated.timing(lightScale, {
        toValue: 1, duration: 900, delay: 180,
        easing: Easing.out(Easing.cubic), useNativeDriver: true,
      }),
    ]).start(() => schedule(() => runTitle(), 260));
  }
        ...
        <View style={styles.shell}>
          ...
          <View style={styles.content}>
            ...
            <View style={styles.illustrationStage}>
              <Animated.View
                pointerEvents="none"
                style={[styles.illustrationLight, { opacity: lightOpacity, transform: [{ scale: lightScale }] }]}
              />
              <Animated.View
                style={[styles.book, { opacity: bookOpacity, transform: [{ scale: bookScale }] }]}
              >
                <View style={styles.bookSpine} />
                <View style={[styles.bookLine, { width: '58%' }]} />
                <View style={[styles.bookLine, { width: '42%' }]} />
                <View style={[styles.bookLine, { width: '50%' }]} />
              </Animated.View>
            </View>
            ...
            >
              Voici comment Zainly va t'accompagner.
            </Animated.Text>

            <Animated.Text
              style={[styles.subtitle, { opacity: subtitleOpacity, transform: [{ scale: subtitleScale }] }]}
            >
              En moins d'une minute, découvre comment Zainly t'aidera à mémoriser le Coran plus sereinement.
            </Animated.Text>
...
const styles = StyleSheet.create({
  ...
  illustrationStage: {
    width: '100%', height: 160, alignItems: 'center', justifyContent: 'center', marginBottom: 42,
  },
  illustrationLight: {
    position: 'absolute', width: 200, height: 200, borderRadius: 140,
    backgroundColor: 'rgba(198,161,91,0.30)',
  },
  book: {
    width: 108, height: 84, borderRadius: 14, backgroundColor: '#FFFDF7',
    borderWidth: 1, borderColor: 'rgba(22,48,38,0.08)',
    paddingVertical: 16, paddingHorizontal: 18,
    shadowColor: SPLASH_GREEN, shadowOpacity: 0.10, shadowRadius: 16,
    shadowOffset: { width: 0, height: 8 }, elevation: 5,
  },
  bookSpine: {
    position: 'absolute', left: '50%', top: 10, bottom: 10, width: 1,
    backgroundColor: 'rgba(22,48,38,0.12)',
  },
  bookLine: {
    height: 2, borderRadius: 1, backgroundColor: SPLASH_GOLD_DIM,
    opacity: 0.5, marginBottom: 10, alignSelf: 'flex-start',
  },
